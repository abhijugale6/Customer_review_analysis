...
def extract_all_reviews(url, clean_reviews, org_reviews,customernames,commentheads,ratings):
    ...
    # customernames_ = page_html.find_all('p',{'class':'MztJPv'})
...
def result():    
    ...
    all_reviews_url = page_html.find_all('div', {'class': 'col pPAw9M'})[0]
    ...
    # plt.savefig('Customer_review\static\images\woc.png')
    ...
    negative_words = ["bad", "cheap","Terrific", "poor", "terrible","waste", "decent"] 
    for i in d:
        if i['stars'] != 0 or any(word in i['review'].lower() for word in negative_words):
            if i['stars'] in [1, 2] or any(word in i['review'].lower() for word in negative_words):
                i['sent'] = 'NEGATIVE'
            else:
                i['sent'] = 'POSITIVE'
